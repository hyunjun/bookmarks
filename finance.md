...
* [금융 AI 에이전트, 정확도만으로 신뢰할 수 있을까? 최근 연구 5편으로 살펴보는 핵심 평가 기준 | digitalbourgeois](https://digitalbourgeois.tistory.com/3669)
  * [2608.11047 V-FiLLM: Verified Financial LLM Reasoning Benchmark](https://arxiv.org/html/2608.11047v1)
    * 실제 표에 근거한 실행 가능 계산 트리에서 금융 추론 벤치마크를 생성—기호적으로 평가해 **정답이 구성상 보장되는(correct by construction)** 문항을 만듦
  * [2606.29251 When Summaries Distort Decisions: Information Fidelity in LLM-Compressed Financial Analysis](https://arxiv.org/html/2606.29251v3)
    * LLM이 금융 자료를 압축(요약)할 때 원문이 지지하던 투자 판단 자체가 바뀔 수 있다는 문제를 '정보 충실성(information fidelity)'으로 정식화—**요약이 결정을 바꾸면 충실성을 잃은 것**
  * [2605.23955 From Accuracy to Auditability: A Survey of Determinism in Financial AI Systems](https://arxiv.org/html/2605.23955v4)
    * 신용 리스크·사기 탐지·자금세탁방지 같은 규제 환경에서의 재현성 서베이—딥러닝·생성형 AI가 하드웨어·아키텍처에 뿌리를 둔 **기계적 비결정성**을 들여왔다는 진단, 정확도에서 감사 가능성으로의 관점 이동
  * [2605.03310 Coordination as an Architectural Layer for LLM-Based Multi-Agent Systems](https://arxiv.org/html/2605.03310v1)
    * 멀티 에이전트 LLM 시스템의 프로덕션 실패율이 41~87%인데 **대부분 기반 모델 능력이 아니라 조정(coordination) 결함** 때문—조정을 별도 아키텍처 계층으로 다뤄 설정과 실패 모드 시그니처의 대응을 원리적으로 제시
  * [2606.29771 CLQT: A Closed-Loop, Cost-Aware, Strategy-Consistent Benchmark for Diagnostic Evaluation of LLM Portfolio-Management Agents](https://arxiv.org/html/2606.29771v3)
    * 고정 구간 수익률 리더보드는 프로세스의 건전성도 우위의 지속성도 보증하지 못함(한 구간 수익은 시장 경로가 지배, 룩어헤드 편향·거래 비용을 통제하면 알파가 사라질 수 있음)—폐루프·비용 인지·전략 일관성을 진단하는 벤치마크 CLQT 제안
  * 5편을 관통하는 리뷰의 질문—정답률을 넘어 **계산의 검증 가능성, 요약의 충실성, 재현성·감사 가능성, 협업 구조, 평가 방식**까지 봐야 금융 AI 에이전트를 신뢰할 수 있다는 프레임
...
* [awesome-trading-agents: Curated list of LLM-driven trading agents, MCP servers, and agent skills for market research, strategy, and execution](https://github.com/LLMQuant/awesome-trading-agents)
  * [LLM 기반 트레이딩 오픈소스 총정리: Agents, MCPs, Skills로 보는 AI Trading 생태계 | digitalbourgeois](https://digitalbourgeois.tistory.com/3616)
  * LLM 기반 트레이딩 에이전트·MCP 서버·에이전트 스킬 큐레이션. 시장 리서치·전략·실행. CC0
...
* [FinanceMCP: AI Agent에게 통일·라우팅·추적 가능한 멀티마켓 금융 데이터를 제공하는 MCP 서버](https://github.com/guangxiangdebizi/FinanceMCP)
  * Tushare·Qveris·Binance API를 통합한 19개 MCP Tools—주식·펀드·채권·거시경제 지표·스테이블코인·가상화폐 데이터와 금융 뉴스·중국 국가통계국 데이터 조회. stdio+Streamable HTTP 지원, npm `finance-mcp`, Smithery 등록. TypeScript, MIT
...
* [kb-watchlist-app: KB 증권 관심종목 및 실시간 시세 조회 앱](https://github.com/f1772man/kb-watchlist-app)
  * KB증권 Open API와 연동해 국내 주식 관심종목을 관리하고 실시간 시세(현재가·전일대비 등락률·시가/고가/저가·누적거래량)를 조회하는 Android 네이티브 앱(Kotlin/Jetpack Compose)+클라우드 중계 백엔드(Cloud Run·Node.js/Express/TypeScript). Firebase Auth·Firestore(관심종목, KB 토큰 24h 캐시)·Secret Manager 구성, 시세 5초 인메모리 캐시. Google Stitch 디자인 시스템(딥 네이비&골드), 상승 빨강·하락 파랑의 한국 시장 관례
  * [KB Open API로 관심종목 시세 앱 만들기 | Easy Coding - YouTube](https://www.youtube.com/watch?v=_g185WwaGV4)
    * 코드를 모르는 초보 개발자가 Google Antigravity 에이전트에게 "말로" 설명해 위 앱을 완성한 과정(골든시그널 초보 개발자 시리즈, 7분). 사람이 한 일은 ①어떤 앱을 만들지 마스터 프롬프트 한 장에 적기(목표·구조·단계별 할 일·검증 방법, KB증권 토큰 발급 /oauth2/token·현재가 조회 IVU10140 요청 형식 원문, 문서 링크와 함께 "짐작하지 말고 확인" 지시, 한글 경로·줄바꿈으로 400 오류 났던 경험) ②본인 인증이 필요한 일—계좌·API 사용신청·키 발급을 0단계로 두어 에이전트가 멈추고 묻게 하고, 키는 채팅에 넘기지 않고 직접 Secret Manager에 등록 ③계획서 승인과 결과 확인
...
* [headless — Financial Data API](https://h6s.ai/)
  ...
  * [#fintech #api #headlessfinance #금융데이터 #스타트업 #h6s #openbanking #developerexperience #aiagent | 진태양](https://www.linkedin.com/posts/taeyang-jin_fintech-api-headlessfinance-share-7449376598497136640-nk-a)
* [Fintech Engineering Handbook](https://w.pitula.me/fintech-engineering-handbook/)
  * [핀테크 엔지니어링 핸드북 | GeekNews](https://news.hada.io/topic?id=30892)
  * 돈을 다루는 시스템 설계 3원칙(없는 돈 만들지 않기·데이터 잃지 않기·무엇도 신뢰하지 않기). 화폐 표현(float 회피·BigDecimal/정수 최소단위), 복식부기, 거래 시점 모델(value/booking/settlement), 감사 추적, 멱등성, 자금 예약 패턴, 웹훅 처리, 정산(reconciliation), 접근 제어
