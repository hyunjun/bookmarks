...
* [Dev Harmony: Communication and Proven SRE Practices | gotopia.tech](https://gotopia.tech/articles/297/dev-harmony-communication-and-proven-sre-practices)
  <details>
    <summary>개발자 경험, 협업, SRE 전략(Gemini 1.0 pro)</summary>
  ...
  <details>
    <summary>Claude 3 Sonnet</summary>

    * Text playground
      * 개발자의 행복감은 작업을 방해하지 않고 효율적으로 수행할 수 있게 해주는 도구와 원활한 커뮤니케이션, 지식 공유, 팀 간 워크플로우 최적화에서 비롯됩니다.
      * "개발하면 운영한다"는 DevOps 철학은 개발자가 프로덕션 코드를 소유하고 운영 역량을 기르도록 합니다. 하지만 개발자들은 변경 사항을 자신 있게 배포하고 문제 발생 시 신속히 해결할 수 있는 적절한 도구와 관측 가능성이 필요합니다.
      * 앙상블/협업 프로그래밍, 코드 리뷰, 공유 코드 소유권은 지식 공유와 팀 전체가 코드베이스를 지원할 수 있는 능력을 높입니다.
      * 당직 엔지니어를 지정해 중단 상황을 처리하게 함으로써 다른 개발자들의 집중력을 높일 수 있습니다.
      * 소프트웨어 의존성 관리는 시스템 진화에는 기여하지 않지만 품질 저하를 막기 위해 필요한 작업입니다. 자동화를 통해 수고를 줄일 수 있지만, 맹목적인 자동 업데이트는 취약점이나 맬웨어를 끌어들일 수 있습니다.
      * 사이트 신뢰성 엔지니어링(SRE) 모델은 운영 팀에서부터 개발팀과 협업하는 내재된 SRE 또는 컨설팅 SRE로 계속 진화하고 있습니다.
      * 내부 플랫폼 구축 시에는 개발자 요구사항을 이해하고, 의견을 수렴하며, 개발자 옹호자를 통해 채택을 권장하는 등 제품 마인드가 필요합니다.
...
이 대화는 개발자의 생산성과 행복을 위해서는 적극적인 커뮤니케이션, 팀 간 협업, 개발 환경 개선이 중요함을 강조하고 있습니다.
    * Chat playground
      ...
      * 이 대화에서는 개발자 경험 최적화, 효율적인 팀 의사소통 관행, 소프트웨어 의존성 관리, 그리고 점점 진화하는 SRE(Site Reliability Engineering) 팀의 역할 등을 다룹니다. 주요 내용은 다음과 같습니다:
        ...
        * 앙상블 프로그래밍과 협업 코드 리뷰 권장
        * 지식 공유를 위한 프로세스 문서화 및 설명이 포함된 테스트 작성
        * SRE 실무를 통합하기 위한 내재형 SRE, 컨설팅형 SRE 모델 탐구
        * 내부 플랫폼을 사용자 조사, 제품 관리, 개발자 옹호가 필요한 제품으로 취급
      * 전반적으로 이 대화는 열린 의사소통, 지식 공유, SRE 실무 도입의 필요성을 강조하며, 이를 통해 팀 효율성을 높이고 영향력 있는 소프트웨어 개발 성과를 달성할 수 있음을 보여줍니다.
  </details>
...
* [하드웨어가 세상을 집어삼키고 있다 – Interpreting Compiler](https://interpiler.com/2024/03/04/%ed%95%98%eb%93%9c%ec%9b%a8%ec%96%b4%ea%b0%80-%ec%84%b8%ec%83%81%ec%9d%84-%ec%a7%91%ec%96%b4%ec%82%bc%ed%82%a4%ea%b3%a0-%ec%9e%88%eb%8b%a4/)
...
* [데일리 스크럼 : '데일리 스크럼'을 더 잘하기 위한 생각 - 컬리 기술 블로그](https://helloworld.kurly.com/blog/daily-scrum-thinking/)
* [왜 당신의 일일 스탠드업 미팅은 실패하는가 | GeekNews](https://news.hada.io/topic?id=8352)
...
* [**Estimating Software Projects - Jacob Kaplan-Moss**](https://jacobian.org/series/estimation/)
  * [Software Estimation Is Hard. Do It Anyway. - Jacob Kaplan-Moss](https://jacobian.org/2021/may/20/estimation/)
    * Claude 3 Sonnet v1
      * 소프트웨어 추정은 어렵습니다. 하지만 여전히 해야 합니다.
      * 한 연구에 따르면 IT 프로젝트의 6분의 1이 200% 이상의 비용 초과와 거의 70%의 지연이 있었습니다. 또 다른 연구에 따르면 IT 프로젝트는 평균 예산의 45%, 일정의 7%를 초과합니다. 대규모 소프트웨어 프로젝트는 더욱 열악하여 1,500만 달러 이상의 예산인 경우 평균 66%의 비용 초과와 33%의 일정 초과가 발생했습니다.
      * 소프트웨어 프로젝트 기간을 추정하는 것은 항상 호프슈타터의 법칙에 부딪힙니다. "호프슈타터의 법칙을 고려하더라도 예상보다 언제나 더 오래 걸린다." 하지만 추정하기 어려운 것을 보고 포기해서는 안 됩니다. 제대로 된 추정치를 내놓고 그에 따르는 능력은 기술 경력 발전에 필수적입니다.
    * Claude 3 Haiku v1
      * 소프트웨어 추정은 어렵다. 그럼에도 불구하고 추정을 해야 한다.
      * 소프트웨어 프로젝트 추정의 어려움은 잘 알려져 있다. 한 연구에 따르면 IT 프로젝트의 1/6이 200% 이상의 비용 초과와 약 70%의 지연을 겪었다. 다른 연구에 따르면 IT 프로젝트는 평균 45% 예산 초과, 7% 지연을 겪는다. 특히 큰 규모의 소프트웨어 프로젝트는 더 나쁜 것으로 나타났다.
      * 그러나 추정의 어려움에도 불구하고 추정은 반드시 필요하다. 타부서와의 협력, 고객과의 약속 등 많은 상황에서 추정이 요구되기 때문이다. 또한 추정 능력은 기술 경력 발전에 중요한 요소이다.
    * [소프트웨어 프로젝트 추정의 어려움과 중요성 알기](https://lilys.ai/digest/397367?sId=c8ea91bd-5976-4665-ac48-69ef1f833bce&source=text&result=summaryNote&s=1)
      * 1. **소프트웨어 프로젝트 예상**이 어렵지만 중요함
        * 소프트웨어 프로젝트의 예상이 어렵다는데는 합의된 바이며, 여러 연구에서 IT 프로젝트가 *예산 초과*와 *일정 지연*을 겪는다는 결과를 확인했다.
        * 특히 *15백만 달러* 이상의 예산을 갖는 소프트웨어 프로젝트는 *예산 초과율이 66%*에 달하고 일정 지연율도 *평균 33%*를 기록했다.
        * 프로젝트의 **소요 시간 추정은 어렵지만** 중요하며, 이 패턴을 인식하지만 좌절하지 않고 계속 노력해야 한다.
      * 2.  *소프트웨어 프로젝트에서의 예측과 미추일 'No Estimates' 주장*
        * 일부 애자일 방법론은 *타임 스케일 단위의 추정을 회피*하기 위해 의도적으로 설계된 임의의 스코어링 시스템을 활용한다.
        * *'No Estimates'* 접근은 일정에 대한 대화를 개선하지만, 언젠가는 정확한 일정이 필요한 상황이 발생할 수 있다.
        * 판매 부서가 새로운 기능에 일정을 약속하면 주요 거래를 체결할 수 있으며, 파트너팀이 작업 일정을 알아야 하는 경우 등 상황에 따라 정확한 답변이 반드시 필요할 수 있다.
      * 3. 기술 경력에서 정확한 추정이 중요함
        * 기술 경력에서 정확한 추정 방법을 배우는 것이 전진하는 비결 중 하나로, *시간대를 감추지 않는* 것이 신뢰를 쌓는 데 도움이 됨.
        * 추정을 피하고 타이트한 일정을 제시하는 방법을 배우지 않으면 경력에 제한이 생길 수 있음.
        * 열린 스케줄과 신뢰를 구축하는 것은 중요하며, 이는 개인 기여자나 엔지니어링 매니저 모두에게 적용됨.
        * 예측력은 배울 수 있는 기술이며, 프로젝트를 세부화하고 타임라인 추정을 하는 습관을 들이면 능숙해질 수 있음.
        * 이를 통해 당신의 팀의 갇히는 부분과 작업을 빠르게 해내는 부분, 코드베이스의 수정이 용이한 영역, 개인적으로 가장 추정이 많이 일어나는 부분을 파악하고 개선할 수 있음.
    * Claude 3 Sonnet v1
      * 이 기술은 작업을 덜 복잡한 작업으로 나누고, 불확실성을 추정하여 예상 시간과 최악의 시나리오 시간을 계산합니다. 필요한 경우 추가적인 작업 분할과 불확실성 감소를 위해 시간 투자를 해야 합니다. 마지막으로 실제 소요 시간을 추적하여 향후 예측 능력을 향상시켜야 합니다.
    * Claude 3 Haiku v1
      * 소프트웨어 추정 기법
        * 지난번 글에서는 소프트웨어 프로젝트 추정이 어렵지만 여전히 필요하다는 점을 설명했다. 이번에는 필자가 사용하는 추정 기법을 자세히 소개한다.
        * 필자의 추정 기법의 핵심은 시간과 불확실성을 모두 고려하는 것이다. 단순히 작업 시간만 예측하는 것이 아니라 최악의 경우를 고려하여 추정하는 것이 중요하다.
        * 구체적인 추정 기법은 다음과 같다. 먼저 작업을 작고 단순한 태스크로 분해한 뒤, 각 태스크의 기대 시간과 최악의 경우 시간을 추정한다. 이를 바탕으로 전체 프로젝트의 예상 기간과 최악의 경우 기간을 산출한다. 필요한 경우 불확실성이 큰 태스크를 추가로 분해하고 시간을 재추정할 수 있다. 마지막으로 실제 소요 시간과 비교하여 추정 능력을 개선한다.
    * [효과적인 소프트웨어 추정 기법과 기간 불확실성을 포착하기](https://lilys.ai/digest/397380?sId=0bcbbca9-55e8-4fbb-bb08-2343998042cf&source=text&result=summaryNote&isBlogRequested=false&s=1)
      1. 프로젝트 타임라인 추정 기술 및 불확실성 캡처 방법
        * 소프트웨어 프로젝트 타임라인 추정이 어렵더라도 중요하며, 불확실성도 함께 고려하는 추정 기술 소개.
        * 작업을 단순화하여 예상 및 최악의 경우 추정을 도출하고, 정확성 추적을 통해 지속적으로 개선하는 전략 소개.
        * 복잡성에 따라 작업을 분해하고 불확실성을 예상에 반영하는 방법, 시간 추정과 불확실성 측정인 '예상 시간'과 '최악 경우 시간'을 함께 고려하는 중요성 강조.
        * 시간 추정과 마찬가지로 낮은 불확실성 여부를 목표로 설정하며, 높은 불확실성이 많다면 추정을 반복해야 하는 신호로 해석.
      2. 프로젝트 복잡성과 불확실성 간 관골 추정과 지속적 계발 방향성
        * 프로젝트의 복잡성과 불확실성을 고려한 계산이 중요하며, 사용 예상 및 최악 경우 일정을 제공한다.
        * 불확실성이 큰 'reticulate splines' 작업이 대부분의 분산을 차지함을 확인하며, 이는 프로젝트의 다수 임무가 낮은 불확실성을 가진 상태로 나타날 수 있다.
        * 다수 임무가 높은 복잡성을 갖는 경우, 이를 더 작은 임무로 세분화하고, *극도 불확실* 임무의 불확실성을 줄이기 위한 방법을 찾을 필요가 있다.
        * 잠재적인 솔루션을 탐색하고 소프트웨어를 작성함으로써 *스파이크*를 활용하거나, 이를 활용해 *더 나은 추정*을 도출해야 한다.
        * 향후 프로젝트 추정을 위해 실제 시간을 추적하여 *피드백 루프*를 형성하고, 추정 기술을 지속적으로 개선해야 한다.
      3. PERT 및 Evidence-based 스케줄링에 대한 접근 방식 비교
        * PERT는 최소, 최대 및 가장 가능성 있는 세 가지 추정을 요구하며, 이를 바탕으로 PERT 예측을 계산한다.
        * 반면 evidence-based 스케줄링은 추정부터 시작하는 대신 직접 작업에 뛰어들고 나중에 추정을 자연스럽게 얻기 위해 이전 작업 시간을 활용한다.
        * Evidence-based 스케줄링은 예측을 일으키는 데 도움이 되지만, 안정된 팀, 일정한 작업, 충분한 데이터를 제공할 수 있는 장기 프로젝트가 필요하다.
        * 이에 비해 PERT는 좀 더 기간적인 측면에서의 추정에 중점을 둔다.
      4. 과일 크기를 활용한 프로젝트 복잡도 측정 방법
        * 다수 팀은 **임시적**이거나 프로젝트가 **다양하고 단기간**이어서 이 기법을 잘 활용하지 못했다.
        * 과일 사이즈 대신 **과일별 복잡성/불확실성 쌍**을 매치해 **시간 추정**하는 '과일 샐러드 스크럼'은 하지만 **유익**하다.
        * 이 방법은 **불확실성**을 캡처하는데 효과적이며 **복잡성과 불확실성**이 함께 **증가**한다.
        * 과일 별 **복잡성/불확실성**을 매핑하고 **수학적 계산**을 통해 시간을 추정하며, **재미**를 더해 **순환**하는 가치를 과소평가하지 말아야 한다.
  * [The art of the SWAG - Jacob Kaplan-Moss](https://jacobian.org/2021/jun/2/swag-estimates/)
    * Claude 3 Sonnet v1
      * 간단한 문제에 대해서는 이전 경험을 바탕으로 직관적으로 대략적인 예측(SWAG)을 할 수 있습니다.
      * 그러나 복잡한 문제, 제약조건이 있는 프로젝트, 예측하기 어려운 상황에서는 SWAG를 피해야 합니다.
      * 중요한 경우 SWAG 후에는 적절한 추정 작업을 수행하여 SWAG의 정확성을 확인해야 합니다.
    * Claude 3 Haiku v1
      * 이 글은 소프트웨어 프로젝트에서 정확한 추정이 중요하다는 것을 설명하고, 빠르고 대략적인 추정인 SWAG(Simple Wild-Ass Guess)에 대해 자세히 설명한다.
      * SWAG는 직관적인 프로세스이며, 과거의 경험을 바탕으로 복잡하지 않은 문제에 대해 빠르게 추정할 수 있다.
      * 그러나 복잡한 문제나 제약이 있는 경우에는 SWAG보다는 더 자세한 추정이 필요하다.
    * [빠르고 정확한 소프트웨어 프로젝트 평가: SWAG의 예술](https://lilys.ai/digest/397411?sId=98357628-90f9-4c97-94da-65d08e04a910&source=text&result=summaryNote&isBlogRequested=false&s=1)
      * 1. 소프트웨어 추정의 예술: 빠르고 정확한 SWAG 예측 기술
        * 소프트웨어 프로젝트에서 *예측*의 중요성과 정확한 예측을 만드는 방법에 대해 소개.
        * 정확성보다 빠르게 예측하는 것이 중요할 때, 일반적으로 SWAG(그럴듯한 예측)이 도움이 될 수 있음.
        * 예측하는 상황이 과거의 유사한 경험에서 비롯된 경우, SWAG가 적절함.
        * 적절한 환경에서 SWAG는 유용하나 너무 정확한 예측은 필수적이지 않으며, 이러한 빠른 추정은 실제로 많은 복잡한 논의를 단순화하고 작업을 원활하게 진행할 수 있음.
      * 2. 강문식에서 '체크리스트 매니페스토'를 인용, 복잡한 문제를 세 가지 유형으로 분류.
        * '체크리스트 매니페스토'에서 Atul Gawande는 문제를 간단한, 복잡한, 복합적인 세 가지 유형으로 분류하는 연구를 인용한다.
        * 간단한 문제는 *케이크 만들기와 같이 레시피와 몇 가지 기본 기술이 있는* 문제이며, 레시피를 따르면 성공 가능성이 높다.
        * 복잡한 문제는 *달에 로켓을 보내는 것과 같이 복잡*하며, 여러 팀과 전문 지식이 필요하며 예기치 못한 어려움과 시간, 조정이 중요하다.
        * 복합적인 문제는 *아이를 키우는 것과 같이 각각의 아이가 독특한 개성*을 가지며, 성공이 매우 불확실하며 다음 아이에 대한 성공을 보장하지 않는다. SWAG를 하기 전에 문제 유형을 파악하는 것이 중요하다.
      * 3. 소프트웨어 엔지니어링에서 예측 어려운 복잡한 프로젝트
        * SWAG는 세부 사항을 모르는 경우, 대부분 틀릴 확률이 더 높다.
        * 본능적인 부정적 감정에 주의를 기울여야 하며, 프로젝트의 복잡성을 인식할 필요가 있다.
        * 프로젝트가 '아 와우' 감정을 일으키게 만드는 종류: *사용자 흐름이 명확하지 않은* 프로젝트, *복잡한 권한 체계가* 포함된 기술적 상황, *고성능 요구사항이* 있는 기술적 상황, *복잡한 기술적 전환*, *복잡한 재구조화*, *다수 이해관계자가 있는* 프로젝트 등이 있다.
        * '아 와우' 느낄 경우, 천천히 하여 철저한 추정을 내는 시간을 투자해야 한다.
      * 4. 정확한 시간 추정 없이 빠르게 예상치를 제시하고, 추가 정보 필요 시 정확한 예측으로 따라가는 방법.
        * 빠르게 *추정치(swag)* 제시하는 방법을 다루며, 정확한 시간 추정은 중요하지 않지만 상황 파악이 바탕이 된다.
        * *추정치(swag)*는 몇 일, 몇 주, 몇 달 또는 분기 단위로 표현하고, 가늠하기 어려운 정도로 표현해야 한다.
        * 가능하면 어려운 작업에 대해 ‘수 년이 걸릴 것’과 같이 구체적으로 설명하며, *정확성을 넘어 정도를 표현*해야 한다.
        * '*어쩌면*', '*아마*', 혹은 의문문으로 불확실성을 표현하는 것이 중요하며, 정확한 추정이 필요한 경우 해당 내용에 대한 추가 정보를 제공해야 한다.
      * 5. 프로젝트 추정 시간 중요성과 SWAG 활용 방법.
        * 프로젝트 추정 시간이 중요할 때, 정확한 추정이 필요하며 문제가 발생할 경우 반드시 추적해야 함.
        * 예를 들어, 회의에 제때 발표할 것이라고 했을 때 정확한 추정을 위해 노력하고, 추정이 유지될지 여부를 계속 업데이트해야 함.
        * *SWAG*를 활용한 프로젝트를 추정했을 경우 결과가 좋으면나쁨이든 공유를 통해 배울 수 있음.
  * [So you messed up. Now what? - Jacob Kaplan-Moss](https://jacobian.org/2021/jun/8/incorrect-estimates/)
    * Claude 3 Sonnet v1
      * 프로젝트가 지연되고 있다는 사실을 인정하고 이를 조기에 공유하는 것이 중요합니다.
      * 마법적 사고(magical thinking)를 피하고 현실을 직시해야 합니다. 늦어진 프로젝트는 대부분 더 늦어지기 마련입니다.
      * 새로운 추정치를 만들되, 이전 추정치도 보관하여 나중에 실패 원인을 분석하고 개선할 수 있도록 합니다.
    * Claude 3 Haiku v1
      * 이 글은 프로젝트 추정이 잘못되었을 때 어떻게 대응해야 하는지에 대해 설명한다.
      * 잘못된 추정을 빨리 인정하고 이를 적극적으로 소통하는 것이 중요하다.
      * 프로젝트가 늦어지는 상황에서 때를 놓치지 않고 새로운 추정을 수립하되, 기존 추정도 함께 보관해야 한다.
  * [Breaking Down Tasks - Jacob Kaplan-Moss](https://jacobian.org/2024/mar/11/breaking-down-tasks/)
    * Claude 3 Sonnet v1
      * 프로젝트를 구성 작업으로 분해하는 반복 프로세스를 설명합니다.
      * 작업이 "충분히 정의되었는지" 판단하는 기준을 제시합니다.
      * 작업 분해 능력은 연습을 통해 향상될 수 있는 기술이라고 말합니다.
    * Claude 3 Haiku v1
...
* [‘애자일 관행’이 ‘디지털 재앙’으로 변할 때 나타나는 5가지 징후 - CIO Korea](https://www.ciokorea.com/news/308593)
...
* [tRPC - Move Fast and Break Nothing. End-to-end typesafe APIs made easy. | tRPC](https://trpc.io/)
  * [trpc: 🧙♀️ Move Fast and Break Nothing. End-to-end typesafe APIs made easy](https://github.com/trpc/trpc)
    * [Node.js Korea | 2022 JavaScript Rising Stars의 Back-end/Full-stack 분야에 **tRPC**가 급부상하였습니다 | Facebook](https://www.facebook.com/groups/nodejskr/posts/5191660667600848/)
  * [Is tRPC The End Of REST/GraphQL? - YouTube](https://www.youtube.com/watch?v=lxnPMB0Jc7E)
...
* MVC
  ...
  * [Java MVC 구조 이해하기 - YouTube](https://www.youtube.com/watch?v=E7dYmcWxQFg)
* MVI
...
* [practice - **무중단 서비스를 위한 아키텍처 설계**](https://www.notion.so/1003/Architecture-8a5899885f2245e69f6123bab1a5d261?pvs=4#3510def05fb449cc9164e3ee08ee983e)
...
* [맥락은 어떻게 표현할 것인가?](https://brunch.co.kr/@graypool/542)
* [경계 설정은 소프트웨어 설계의 핵심 활동](https://brunch.co.kr/@graypool/543)
* [Context와 컴포넌트와 이상적인 아키텍처](https://brunch.co.kr/@graypool/544)
...
* [도메인 이벤트 처리 구조 개발의 시작](https://brunch.co.kr/@graypool/809)
...
* [MSA 이벤트 드리븐 아키텍처(Event Driven Architecture) - outbox 패턴, CDC](https://blog.naver.com/pjt3591oo/223308097649)
...
* [**GOTO 2020 • When To Use Microservices (And When Not To!) • Sam Newman & Martin Fowler - YouTube**](https://www.youtube.com/watch?v=GBTdnfD6s5Q)
  ...
  * [VidiGo When To Use Microservices (And When Not To!) • Sam](https://vidigo.ai//chatbot/summary/Tj8fZQg3pVcOsUl)
    * 4. 데이터와 조직 문화의 중요성
      * 복잡한 시스템을 분해할 때 가장 큰 복잡성은 데이터에
      * 마이크로서비스를 추출한 후, 기존 데이터베이스 시스템 사용 부분 이해 필요
      * 소프트웨어 개발에서 데이터보다 더 복잡한 것은 사람들이며, 마이크로서비스를 고려할 때 스킬 차이와 문화 차이가 중요
  * [마이크로서비스를 사용해야 할 때(그리고 사용하지 말아야 할 때!) - 샘 뉴먼 & 마틴 파울러 - GOTO 2020 | 완벽한 영상요약, 릴리스에이아이 | Lilys AI](https://lilys.ai/digest/384987?sId=GBTdnfD6s5Q)
    * 2.💻마이크로서비스 아키텍처 선택 기준과 장점
      * 마이크로서비스를 사용해야 하는 매우 유용한 이유가 있을 때 사용해야 함
      * 마이크로서비스 아키텍처는 확장 가능한 애플리케이션에 대한 옵션을 제공하며, 독립적인 배포 가능성과 장애 표면 격리를 통해 애플리케이션의 견고성을 향상
      * 마이크로서비스 아키텍처는 옵션을 제공하며 선택하고 구현할 때 원하는 결과를 얻을 수 있는 의식적인 선택
      * 마이크로서비스 아키텍처에 대한 선택이 비용이 든다는 것을 강조하며, 새로운 기술을 사용하기 위한 것이 아닌 원하는 결과를 얻기 위한 수단으로 사용해야 한다고 주장
    * 3.💻마이크로서비스 아키텍처와 모놀리식 아키텍처의 관계, 이점 및 고려해야 할 사항
      * 모놀리식 아키텍처와 마이크로서비스 아키텍처를 구분하기 쉬워서, 일부는 마이크로서비스 아키텍처를 더 유용하게 여기나, 모노리식 아키텍처도 분산 시스템
      * 맥스는 단일 프로세스 모놀리식 아키텍처조차 '분산 시스템'으로 여겼으며, 또한 '분산 시스템은 어렵다'고 말하나, 이를 단순화하여 이해할 수 있다는 점 강조
      * 마이크로서비스 아키텍처를 도입할 때, 독립 배포 가능성이 높은 구조를 추구하여 효율적으로 시스템을 운영할 수 있도록 노력하는 것이 중요
      * 마이크로서비스를 도입하기 전에, 실행 가능성과 이점을 고려하여 단일 프로세스 모놀리식으로부터 서비스 기반 아키텍처로 진화시킬 수 있는 방법을 탐색하며, 진화시키는 것이 바람직
      * 실패를 두려워하지 말아야 하며, 마이크로서비스를 저비용으로 사용해보고 그 결과를 관찰하는 것이 중요
        * 새로운 시스템을 구현할 때 미리 시간과 비용을 들어서만이 아니라, 적절한 시기와 방법을 고려하여 효과적으로 진행 필요
    * 마이크로서비스의 핵심적 가치로는 제로 다운타임, 독립적 배포성, 데이터 및 처리 분리가 있으며, 이를 통해 조직 구조가 더 잘 반영
    * 5.✨마이크로서비스 디자인 원칙: 정보 은폐, *독립 배포성*의 중요성
      * 마이크로서비스 변경이 어려울 때, 패턴을 찾아야. 서로 함께 변경되는 마이크로서비스 모음을 찾고, Jira와 같은 티켓 추적 도구를 활용하여 커밋을 작업에 연결 가능
      * 매월 완료한 스토리와 커밋을 확인하여 어떤 서비스에 영향을 미치는지 파악 가능
      * 서로 변경되는 마이크로서비스 패턴을 발견하면 통합하거나 다양한 방식으로 잘게 나누는 것을 고려 필요
      * 독립 배포성의 중요한 디자인 원칙은 정보 은폐, 마이크로서비스 내부에 정보를 최대한 은폐함으로써 인터페이스를 줄여 변경을 보다 용이하게
    * 6.💻Fllickr와 Etsy의 일렬 엔지니어가 설명하는 모놀리식과 마이크로서비스의 장단점
      * 작은 개별 마이크로서비스에서 변경 사항을 수행할 때는 데이터 양이 적어 시스템에 더 낮은 영향을 미칠 것으로 예상
      * 기업 규모 또한 중요하며, 한 팀이 시스템에 작업하는 경우, 고도의 조정과 효율적 소통 가능
        * 그러나 많은 팀이 모놀리식 응용프로그램을 작업할 때는 조정 및 변경을 위한 협력이 더 필요하다고 논의
      * 분산 시스템의 복잡성을 고려할 때, 단일 모놀리식을 테스트하여 전체 시스템을 효과적으로 테스트할 수 있다는 측면에서 모놀리식 배포가 더 어려울 수 있음
    * 7.🛠️마이크로서비스와 모듈화, 프로세스 경계의 중요성에 대한 통찰
      * 마이크로서비스를 사용하면 각 릴리스의 영향을 제한하는 것이 더 쉽기 때문에 독립적인 배포를 지향해야 한다는 주장이 있음
        * 팀 규모가 커질수록 배포를 조정하는 것이 기하급수적으로 어려워진다고 설명
      * 모든 일이 데이터를 다루다 보니 마이그레이션에 시간이 오래 걸리고 한 애플리케이션 내에서 그 데이터를 포함시키기 더 어려움
      * 모듈 간 경계를 설정하고 마이크로서비스를 사용하면 큰 팀에서 필요한 방식으로 일하게 하며, 모노리스 애플리케이션 내의 모듈로 설정해야 한다고 강조
      * 모듈을 서비스로 만들면 모듈을 지키는 것이 더 어려워지며, 프로그래밍 언어 인프라가 모듈성을 제어하는 데 탁월한 역할을 하지 못한다고 지적
      * 모듈을 존중하지 않는다면 모놀리틱 아키텍처를 위반하게 되는데, 이것이 서비스로 변환되면 이를 어기는 것이 훨씬 고통스러워짐
    * 8.💻모놀리딕을 마이크로서비스로 분리하는 것의 어려운 점, 데이터 관리에 대한 관찰
      * 모놀리딕을 마이크로서비스로 분리할 때 가장 어려운 부분 중 하나는 데이터 처리. 관계형 데이터베이스 안의 데이터를 분리하는 것도 복잡
      * 관계형 데이터베이스는 관계에 대한 것이여서 데이터를 분리하면 일부 관계가 깨지게 되는데, 이는 어렵고 복잡한 과정
      * 데이터베이스로 작업할 때 종종 코드보다 데이터 간의 관계가 훨씬 명확. 초기에 스키마에 많은 시간을 쏟았기 때문에 의도와 도메인이 코드보다 잘 전달
      * 이런 상황에서 가장 먼저 하는 일 중 하나는 스키마를 그래픽 도구에 로딩하여 형태를 살펴보고 구조를 분석하는 일
    * 9.💻데이터베이스 시스템에서의 관계성과 미적분적 프로세스 고찰
      * 데이터베이스에서의 외래키 관계는 참조 무결성에 관한 것뿐만 아니라 관계를 명확하게 만드는 것이기도 함
      * 실제로 관계형 데이터베이스에서는 참조 무결성을 강제할 수 있지만 많은 사람들이 이를 실행하지 않음
      * 시스템을 분리하는 과정에서 데이터베이스가 가장 복잡할 것이라고 깨달았고, 마틴빌 닷컴의 투트카가 데이터 중심 마이크로서비스 분리에 관한 글을 써 유용하다고 평가
      * 데이터 이동에 대한 이슈와 동시에 단일 데이터베이스로 다중 스키마를 가지는 것이 중요함을 강조하며, 이는 데이터 분리의 중요한 부분
      * 호스팅 비용 문제와 성능, 데이터 일관성, 데이터 중앙 집중에 따른 문제 해결에 대한 논의가 필요하며, 이러한 문제들을 해결하는 것은 중요하지만 어려운 작업
    * 10.📦마이크로서비스로 전환 시 데이터 복잡성, 조직 및 기술적 어려움 소개
      * 마이크로서비스 분야에서 업계 전반이 다양한 도구 부재로 어려움에 직면하며 케이스 스터디 필요성 강조
      * 마이크로서비스로 전환 시 데이터의 엄청난 복잡성과 진행 어려움에 대한 설명
      * 기존 모놀리식 소프트웨어 관리에 익숙한 조직은 분산 시스템의 도전과 인력 부족에 직면
      * 기존 명령과 통제 방식이 아닌 분산 의사결정 체제를 받아들여야 마이크로서비스 구조의 최대 가치를 누릴 수 있음
      * 마이크로서비스 구조와 일반적인 의사결정 체계 간의 패러다임 변경 및 기존 데이터 활용성 저하에 대한 경고
    * 11.💰마이크로서비스는 짧은 시기에는 비용이 많이 소요되지만, 조직의 변화와 조화되어 혜택을 얻을 수 있음
      * 마이크로서비스 아키텍처는 비용 절감이 아닌 수익 증가에 도움을 줄 수 있지만, 짧은 시기에는 비용이 더 많이 필요
      * 일부 기업은 조직적 자율성을 수용하지 않으면 실질적인 이점을 놓칠 수 있음
        * 기존 방식이 변경되지 않는 기업들도 존재하며, 이는 조직적 변화와 실험적 변화를 촉구해야 한다는 의미
      * 기술적 역량 이동을 염두에 둔 경우, 팀 내 책임 분담을 점진적으로 시도할수록 효과적, 일반적으로 중앙 플랫폼 엔지니어링 팀으로 많은 노력이 집중
      * 마이크로서비스 아키텍처에 시간과 에너지를 소비할 시, 기존의 상하향 체계 조직을 변경하지 않으면, 두 마리 토끼를 잡지 않는다는 점 명심 필요
      * 마이크로서비스를 사용하여 최대의 이점을 얻고자 조직적인 변화에 대한 의지 필요
...
* [대용량 트래픽 처리를 위한 쿠팡의 백엔드 전략. 마이크로서비스로 고객에게 데이터 서빙하기: 고가용성, 고처리량, 그리고… | by 쿠팡 엔지니어링 | Coupang Engineering Blog | Sep, 2022 | Medium](https://medium.com/coupang-engineering/%EB%8C%80%EC%9A%A9%EB%9F%89-%ED%8A%B8%EB%9E%98%ED%94%BD-%EC%B2%98%EB%A6%AC%EB%A5%BC-%EC%9C%84%ED%95%9C-%EC%BF%A0%ED%8C%A1%EC%9D%98-%EB%B0%B1%EC%97%94%EB%93%9C-%EC%A0%84%EB%9E%B5-184f7fdb1367/)
  * 마이크로 서비스에서 대용량 트래픽에도 지연 시간 없이 서비스하기 위해 고가용성을 해결한 후기
* [캐시를 활용한 대용량 트래픽 처리 성능 향상. 마이크로서비스와 캐시 레이어를 함께 운영하며 쿠팡이 겪은 복잡한 기술적… | by 쿠팡 엔지니어링 | Coupang Engineering Blog | Sep, 2022 | Medium](https://medium.com/coupang-engineering/%EC%BA%90%EC%8B%9C%EB%A5%BC-%ED%99%9C%EC%9A%A9%ED%95%9C-%EB%8C%80%EC%9A%A9%EB%9F%89-%ED%8A%B8%EB%9E%98%ED%94%BD-%EC%B2%98%EB%A6%AC-%EC%84%B1%EB%8A%A5-%ED%96%A5%EC%83%81-a274f4731d07/)
  * 마이크로 서비스에서 대용량 트래픽 처리를 위한 Cache 아키텍처 개발로 성능 향상한 후기
* [Our backend strategy to handle massive traffic | Coupang Engineering | Coupang Engineering Blog](https://medium.com/coupang-engineering/our-backend-strategy-to-handle-massive-traffic-d30cd6cc4fb2)
...
* [엔터프라이즈 MSA 이야기 3탄 – SR 도메인 편 :: GS Retail Engineering](https://gsretail.tistory.com/41)
...
* Kalix [Build high-performance, cloud-native microservices and APIs | Kalix.io](https://www.kalix.io/)
...
* [아름다운 코드에 대하여 | kciter.so](https://kciter.so/posts/what-is-beautiful-code)
...
* [해외 연사 비자 지원하기 | Youngbin Han's blog](https://blog.youngbin.xyz/2022-12-24-uca22-visa-and-k-eta/)
...
* [OpenInfra & Cloud Native Day Korea 2022](https://2022.openinfradays.kr/program/sessions)
  * [OpenInfra & Cloud Native Days Korea 2022 - YouTube](https://www.youtube.com/playlist?list=PLkgLtPJ7Lg3rnn_p6QV-GVdgqL-ZgZnZo)
...
* [Qodana: The code quality platform for your favorite CI by JetBrains](https://www.jetbrains.com/qodana/)
  ...
  * [Qodana 무료로 시작해보기 (2023.11)](https://jojoldu.tistory.com/759)
...
* [SHA256 해시 결과가 달라요?](https://velog.io/@joosing/is-the-sha256-hash-different)
  * [practice - SHA256](https://gist.github.com/hyunjun/96dd7f0e738a148412af096dcd445566#file-difference_by_datatype-md)
...
* [Webhooks VS APIs](https://www.linkedin.com/feed/update/urn:li:activity:7112349415893274624/)
...
* [블로그 | 메시, 패브릭, 분산 클라우드?··· 데이터 분야 신개념을 CEO에게 설명하는 방법 - CIO Korea](https://www.ciokorea.com/news/290989)
...
* [작고 아기자기하지만, 개발조직의 문화 만들기 ep 1. 웹프론트개발그룹 운영진을 소개합니다 | 우아한형제들 기술블로그](https://techblog.woowahan.com/9347/)
* [작고 아기자기하지만, 개발조직의 문화 만들기 ep 2. 장기프로젝트 기획하고 운영하기 | 우아한형제들 기술블로그](https://techblog.woowahan.com/9644/)
...
* [Failure Is Always An Option • Dylan Beattie • GOTO 2023 - YouTube](https://www.youtube.com/watch?v=NKQ--vGY35E)
  * [VidiGo Failure Is Always An Option • Dylan Beattie • GOTO](https://vidigo.ai//share/summary/b7432ca4cb56)
    ...
    * 시스템은 초기에 문제가 많고, 중간에 안정되며, 마지막에 다시 문제 발생
      * 사람들은 변화를 두려워하며, 새로운 시스템에 대한 기대로 인해 기존 시스템을 소홀히 취급
    * 스페이스 셔틀 챌린저호의 비극은 점점 감소하는 엄격함 때문
      * 리처드 파인만은 위험 관리 실패를 지적하며, 일상에서도 비슷한 실수를 반복할 수 있다고 경고
    * 호라이즌 it 시스템의 실패는 영국 역사상 가장 큰 사법 오류 중 하나
      * 개발자와 엔지니어는 자신들의 결정이 완벽할 것이라는 착각을 해서는 안됨
    * 이중 인증 시스템이 현실 세계에서 예상치 못한 문제에 직면하는 사례 소개
      * 사용자들은 제한된 환경에서도 문제를 해결하기 위해 창의적인 방법 탐색
      * 기술이 실제 세계에 배치될 때 발생할 수 있는 예상치 못한 상황들을 고려하는 것의 중요성 강조
    * 기술이 어떻게 현실 세계의 다양하고 복잡한 상황에 적응해야 하는지 설명
      * 사용자들은 주어진 도구와 제약 사항 내에서 문제를 해결하기 위해 끊임없이 새로운 방법을 모색
      * 기술 개발자는 사용자가 기술을 어떻게 활용할지 예측하고 이에 맞춰 대응하는 유연성 필요
  * [실패는 언제나 선택 사항입니다 - 딜런 비티 - GOTO 2023 | The fastest way to absorb knowledge | Lilys AI | Lilys AI](https://lilys.ai/digest/437489?sId=NKQ--vGY35E)
    * 2. 인류 최고의 사용자 이야기: 달에 사람을 착륙시키고 안전하게 지구로 귀환
      * 1969년 7월 20일, 아폴로 프로그램으로 달에 인류 착륙 성공
      * NASA는 번개에 맞을 경우 2백만 리터의 로켓 연료가 어떻게 되는지는 시뮬레이션하지 않았지만, 모든 고장 시나리오는 시뮬레이션함
      * 조명이 꺼지고 계기판만 작동 중단, 블랙아웃 상태인데 추청장치를 보조로 바꾸라는 지시에 조작하여 성공적 복구
      * 아폴로 13에서 엔진 고장 외에도 모든 시스템에는 백업이 설계되어 있었음. 최악의 상황을 대비하여 발사탑에서 우주선을 안전하게 구출하는 시스템도 존재
    * 4. 스페이스 셔틀의 컴퓨터 예비 체계
      * 스페이스 셔틀은 IBM ap101 메인프레임을 5대 탑재하여 운영
      * 네 대는 동일한 하드웨어와 소프트웨어를 동일 타이밍으로 운영하여 임무 중 동일한 결과를 내놓게끔 설계
      * 만약 한 대의 컴퓨터에서 이상이 발생하면 다른 세 대로 작동 가능, 두 대 이상 잃으면 두 대가 일치하지 않을 경우 문제 발생 가능성으로 간주
      * 따라서 두 번째로 구동되는 컴퓨터는 완전히 다른 소프트웨어를 실행하여 주요 시스템과 다르게 작동함으로써 장애에 강한 체계 구축
    * 5. 우주선용 부품 제거와 안전
      * 우주선 부품을 운반하는 것은 정말 *비용이 많이 든다*. 너무 조심스럽게 대우해 왔기 때문에 결국 유용한 것들을 더 이상 운반하지 않게 됨
      * 우주 여행보다 *택시 탑승이 위험*한 이유는 택시가 더 많은 사람들을 사고로 인해 사망시키기 때문
      * 우리는 익숙한 것인 택시를 더 안전하게 여기고, 낯선 것인 비행을 더 위험하게 인식
      * 소프트웨어는 초기 수명주기에 문제가 많이 발생하며, 점차적으로 완벽하지 않은 점들이 드러난다. 마지막에는 시스템이 손상되기 시작
    * 6. 2020년도에만 비로소 개장한 베를린 브란덴부르크 공항
      * 베를린의 공항 개장이 예정된 2008년, 실제 개방된 것은 2020년 혹은 2021년으로 연기
      * 신개념에 집중하며 예전 시스템이 무시되는 현상 발생
      * 문제 발생 가능성에 대한 관심 경각심이 감소하며, 새로운 시스템으로의 투자
      * NASA의 우주선 발사 중단 기준 중 일부에는 기상 조건도 포함되었는데, 특히 최저 기온은 미고려
      * 1984년에 우주선이 13°C 기온에서 발사된 사례에서 납품업체들은 추위로 발사가 적합하지 않다고 조언했지만, 결국 발사 진행
    * 7. 우주 비행사의 안전 문화 비유와 클라우드 소프트웨어 개발 비교
      * 1985년 기온 10도, 1986년 6도였을 때 스페이스 셔틀 챌린저가 폭발한 사건으로 점진적으로 감소하는 엄격함이 안전 문제의 중요 요인이 될 수 있음을 인지
      * 노벨상 수상자인 물리학자 리처드 파인만이 조사에 기여한 인물 중 하나였으며 NASA는 그의 의견을 적합하지 않아함
      * 시대가 변화함에 따라 현재는 클라우드 소프트웨어가 빠르고 쉽게 개발되지만, *안전을 의식하는 사고방식*은 중요
      * 시스템을 설계하는 데 있어 안전에 중점을 두는 것이 중요하며, *예기치 못한 문제*에 대비할 필요
    * 8. 클라우드 아키텍처에서의 결정적인 실패
      * 클라우드 아키텍처의 주요 실패 요소로는 *마스터카드 한계*가 있었는데, 이는 Amazon 계정에 사용되었고 결제 문제로 서비스 중단 발생
      * 개발자들이 신용카드 한도를 고려하지 않는 이유는 *신용카드를 자신의 책임으로 생각하지 않기* 때문
      * 시스템적 사고의 부족은 *부분에 집중*하고 *전체 시스템을 고려*하지 않음을 드러냄
      * 개발자와 엔지니어로서 가장 큰 실수는 *오만함과 무신경*에서 비롯. 이는 IT 역사에서 가장 큰 *사법 오류*
    * 9. 컴퓨터 오류와 사용자 사례 비교, 기술 문제와 실제의 약점
      * 일부 사람들은 오해를 했을 수도 있다고 인정하면서, 일부는 신고 취소. 하지만 시스템의 결함으로 이들은 파산
      * 도움 데스크에서 일해본 경험을 통해, 컴퓨터의 오류를 사용자에게 설명받는 상황을 쉽게 이해
      * 실제로는 백업이 없음에도 그들을 위해 백업을 확인하고 파일을 복구하는 경험
      * 기술적 문제와 사용자 경험의 간극이 논의되며, 과도한 보안 시스템의 한계를 들면서 개선이 필요함을 강조
    * 10. 프로그램은 현실세계에서 예상치 못한 문제에 직면할 수 있음
      * 현실세계에선 *코드 동작 예측*의 실패, 까다로운 상황과 *날카로운 문제*가 발생 가능
      * 사람들은 예상치 못한 *도로 점등 환경*을 발견하며 프로그램 예견하지 못할 수도
      * 사용자는 문제를 *창의적*으로 풀려는 성향 존재
      * 예시로, 도처에 숨겨진 업데이트를 전달하기 위해 *핫메일*을 활용하는 기술, 사용자의 *창의적 해결*능력 소개
    * 11. AI 개식별 모델 활용한 창의적 사용법과 아프리카 통신 혁명
      * AI 개식별 모델을 사용해 뉴럴 네트워크를 통해 다양한 분야에 적용
      * 훈련 데이터셋을 뒤집어 개 증폭기로 활용해 문화적 예술과 사회적 변화 창출
      * 아프리카에서 혁명적인 통신 발전, 나이로비의 경제적 성장과 통신 인프라 문제 설명
      * 아프리카의 통신 인프라 구축을 위한 필수 조건: 밀도 높은 인구, 금전적 지원, 안정적인 정부 체제
    * 12. 아프리카에서 휴대전화로 무선 디지털 화폐 이전 방법이 발명
      * 아프리카의 셀룰러 서비스가 이들의 경제적 문제를 해결하기 위한 인내력
      * 사람들은 마을마다 한 대의 휴대전화를 공유하여 홈으로 돈을 전송할 수 있게 한 전화 상점을 운영
      * 전화 크레딧을 통한 돈 이전 방식을 통해 무선 디지털 통화 이전이 발명되었고, 이는 케냐 전역에서 주요한 뱅킹 시스템으로 발전
      * 아프리카 지역에서는 자신을 가족이라고 간주하는 '친구와 가족' 서비스가 도입되어 디지털 경제가 부상
    * 13. 케냐 M-Pesa: 빈 사람에게 돈 지불하지 않게 한 혁신
      * 아프간 경찰은 모든 급여를 M-pesa를 이용해 지급하기로 결정했지만, 60%의 경찰관이 사실상 미존재
      * 이로 인해 케냐에서 M-pesa는 현재까지 존속하며, 안전하게 돈을 벌고 보관하고 이체할 수 있는 혁명적인 방법
      * 약 25만 가구에게 박물로 나가는 길을 제공해 가난에서 벗어날 기회 제공
      * 사용자들은 *시스템에 대한 혁신적인 아이디어*를 제안 가능, 이를 수용하여 문제에 대한 해결책 도출
...
* B급 프로그래머
  ...
  * [8월 1주 소식(개발/클라우드/고성능 서버/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2022/08/b-8-1.html)
  ...
  * [9월 소식(빅데이터/인공지능, 읽을거리 부문)](https://jhrogue.blogspot.com/2022/09/b-9.html)
  * [9월 4주 소식(개발/클라우드/고성능 서버/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2022/10/b-9-4.html)
  * [10월 2주 소식(개발/클라우드/고성능 서버/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2022/10/b-10-2.html)
  * [10월 4주 소식(개발/클라우드/고성능 서버/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2022/10/b-10-4.html)
  * [11월 2주 소식(개발/클라우드/고성능 서버/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2022/11/b-11-2.html)
  * [11월 소식(빅데이터/인공지능, 읽을거리 부문)](https://jhrogue.blogspot.com/2022/11/b-11.html)
  * [11월 4주 소식(개발/클라우드/고성능 서버/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2022/11/b-11-4.html)
  * [12월 소식(빅데이터/인공지능, 하드웨어/블록체인, 읽을거리 부문)](https://jhrogue.blogspot.com/2022/12/b-12.html)
  ...
  * [3월 1주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/03/b-3-1.html)
  ...
  * [4월 2주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/04/b-4-2.html)
  * [4월 3주 소식(빅데이터/인공지능, 하드웨어/블록체인, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/04/b-4-3.html)
  * [4월 4주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/04/b-4-4.html)
  ...
  * [7월 2주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/07/b-7-2.html)
  * [7월 3주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/07/b-7-3.html)
  * [7월 4주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/07/b-7-4.html)
  * [8월 1주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/08/b-8-1.html)
  * [8월 2주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/08/b-8-2.html)
  * [8월 3주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/08/b-8-3.html)
  ...
  * [10월 1주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/10/b-10-1.html)
  * [10월 2주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/10/b-10-2.html)
  * [10월 3주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/10/b-10-3.html)
  * [10월 4주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/10/b-10-4.html)
  * [11월 1주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/11/b-11-1.html)
  * [11월 2주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2023/11/b-11-2.html)
  * [11월 3주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2023/11/b-11-3.html)
  ...
  * [2월 1주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2024/02/b-2-1.html)
  * [2월 2주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2024/02/b-2-2.html)
  ...
  * [3월 3주 소식(개발/설계/경력관리/보안/클라우드/데이터베이스 관련 소식 정리)](https://jhrogue.blogspot.com/2024/03/b-3-3.html)
  * [3월 4주 소식(빅데이터/인공지능, 하드웨어, 읽을거리 부문)](https://jhrogue.blogspot.com/2024/03/b-3-4.html)
...
# Oncall On call
* [Navigating the Chaos: A Holistic Approach to Incident Management by Hila Fish - YouTube](https://www.youtube.com/watch?v=WEviDS-8pO4)
  * oncall에 한정된 이야기는 아니고, 전반적으로 문제 발생시 대처하는 방법에 대한 이야기
  * [VidiGo Navigating the Chaos: A Holistic Approach to Incid](https://vidigo.ai//chatbot/summary/nN7X5Hhdo7fK6nB)
    * 사건 관리: 사고 대응 마인드셋, 구조화된 프로세스, 온콜 특성
    * 사건 발생 후 즉시 메모를 작성해야 효과적인 회의 가능
      * 지식을 공유하고 사고로부터 배워 모두에게 이익이 되어야
    * 인시던트 매니저는 작업을 나누고, 명확하게 지시해야 하며, 필요하지 않은 인원은 줄여야
      * 스트레스 상황에서도 차분하게 대응할 수 있는 능력 중요
      * 도움을 요청하는 것이 중요하며, 이는 비즈니스 이익을 최우선으로 고려하는 것을 의미
    * 권위가 없어도 회의를 관리하고 동료들의 의견을 듣는 방법을 실천 가능
      * 일상에서 리더십을 발휘하면 긴급 상황에서 사람들이 당신의 지시를 따를 가능성 향상
      * 관심과 배려를 통해 동료들과 더 깊은 관계를 맺고, 그들의 동기를 이해하여 협력 끌어내는 게 가능
    * 사건 발생 후 보고서 작성, 변경 사항 검토, 경보 수정 등을 통해 다음 사건을 예방 가능
      * 온콜 교대 시 정보 공유로 팀원들이 최신 상황을 인지하게 하여 생산성 향상
      * 시스템 아키텍처와 애플리케이션 간 움직임을 이해함으로써 잠재적인 실패에 대비하고 취약점을 보완
  * [혼돈 속에서 탐색하기: Hila Fish의 인시던트 관리에 대한 총체적 접근 방식 | 완벽한 영상요약, 릴리스에이아이 | Lilys AI](https://lilys.ai/digest/383472?sId=WEviDS-8pO4)
    * 인시던트를 관리하는 구조화된 과정 중요
      * 비즈니스적 관점을 고려하여 인시던트를 조직적으로 처리하고 그 영향을 최소화 필요
      * 인시던트 관리에서는 문제 식별부터 카테고리화, 인시던트 통보 및 에스컬레이션, 문제 해결까지 과정 필요, 각 단계에서 적절한 조치와 의사소통 필요
      * 인시던트를 처리하며 중요한 질문들을 통해 스트레스를 줄이고 구조화된 접근법으로 이끌어야
    * 알림 및 에스컬레이션 중요. 문제 해결을 위해 다른 팀에 에스컬레이션이 필요한 경우 존재
      * Investigate 단계는 중요한 정보 집중이 필요, 양립할 정보에만 집중해야 함. 또한, 시스템 흐름은 다양한 부분으로 구성
      * 근본 원인을 발견하고 이해하는 것이 중요하며, 표면적인 증상보다 근본 원인 우선시 필요
      * 문제 해결과 복구 단계에서는 가장 빠르고 효과적인 대책 선택 필요. 문제 해결 후 필요한 조치품목 확인하여 재발 방지와 안정적 환경 유지 중요
      * 문제 해결 후 알림 필요. AIO 알림 조정 가능성이 있으며, 재발 방지를 위한 조치 필요
    * 인시던트 런북은 사건을 처리하기 위한 절차로, 판단이 필요한 인시던트 관리에 매우 도움
      * 새로운 문제가 발생했을 때 이를 해결하기 위한 구조화된 프로세스와 문서로 작성 필요
      * 유사한 인시던트 예방을 위해 로그 롤오버를 설명, 문제를 해결하기 위해 log rotate를 사용하는 서버에 대한 티켓 공유 및 전체 서버에 재귀적으로 확인 권장
      * 포스트모템 회의를 통해 인시던트에서 배운 점을 공유하고, 문제 예방을 위해 런북이나 일일 브리핑을 통해 지식을 공유
      * 이외에도 인시던트 관리를 위해 워 룸이 필요하며, 크리티컬한 상황에서 4~5명 이상의 팀 구성원이 모여 해결하는 것이 중요
    * 조용하고 침착한 태도로 업무를 수행하며 직무를 명확히 전달하는 것이 중요
      * 원활한 업무 진행을 위해 인원을 줄이거나 제거할 용의가 있어야 하며, 긴장된 상황에서 너무 많은 사람들이 모여있으면 소란스러울 수 있음
      * 워룸에서 도움을 준 후에는 필요한 경우를 제외하고 나가서 다시 필요할 때 연락을 받아야 함
      * incident 관리자에 필요한 질은 두루두루 있지만, 빠른 판단 능력이 중요하며, 이를 향상시키기 위해 급박한 상황에서 행동을 취하는 것부터 시작하는 것이 필요
      * 중요한 정보와 무관한 정보를 구분하는 능력도 중요하며, 이를 향상시키기 위해 시스템 흐름을 익히는 것이 도움
    * 새로운 상황에서 압박 대응이 중요
      * 스트레스는 통제 불능의 증상이라는 점을 감안하여, 관련 데이터 수집과 체계적 작업이 중요
      * 사고처리에서 순차적으로 작업하고, 도움을 요청하는 것이 중요함을 강조하며, 도움 요청 습관을 길러야 한다고 조언
      * 문제를 해결할 때 자신감을 가져야 하며, '할 수 있다'는 접근법이 중요하다고 강조
      * 자신만의 업무 방식과 즉각적인 도움 요청 습관을 통해 사건 관리와 문제 해결 능력을 향상시켜야 한다는 메시지 전달
    * 담당자에게 연락하여 지원을 요청하고, 효과적인 커뮤니케이션과 팀 협업 중요성 강조
      * 책임을 질 필요성을 강조하며, 커뮤니케이션 가이드라인과 프로세스 축적의 중요성 강조
      * 인시던트 관리에서 리더십과 팀워크 중요성을 강조하며, 고객과 효과적인 커뮤니케이션 필수
    * 리더십 및 관리 능력 향상을 위한 실천 방법과 준비 과정
      * 리드하는 능력을 향상하기 위해 업무 미팅 및 데모를 관리하는 연습이 중요하며, 일상에서 끊임없이 이를 실천함으로써 다른 사람들의 신뢰 획득
      * 예기치 못한 사건이 발생했을 때, 리드할 수 있는 능력을 보여줄 수 있으며, 함께 일하는 사람들, 시스템, 비즈니스에 관심을 갖는 것이 중요하다고 강조
      * 기존 회사들에서 잘 실천되고 있지 않은 것이지만, 근무교대를 통한 정보 공유가 중요, 근무시간 배정에 대한 다양한 방법들에 대해 언급
    * 온콜 쉬프트 종료 시 일어난 상황을 Slack에 요약 공유
      * 팀원들이 이를 통해 프로덕션 상황을 파악하고 개선을 위해 사후조치를 수립하며, 팀 간 지식 교환 및 감사를 위해 작성 필요
      * 사고 발생 시 빠르게 파악하고 효과적인 회의를 위해 사후 정리 노트를 작성해야하며, 자가치유 후보 확인과 같은 자동화 작업 등으로 사고 예방 노력 필요
      * 가짜 양성 경보 수정이 중요, 룩북 이용과 자동화를 통해 향후 사고 대비 필요. 수정이 필요한 경보는 미루지 말고 즉시 조치
    * 인시던트 처리 후, 깊은 지식 전달 및 협업 강조
      * 정례적인 업무 시간에는 로테이션 및 정보 확인, 사고 대응 포인트 확인
      * 인프라뿐만 아니라 전체 시스템 이해와 취약점 파악 중요
      * 응용프로그램의 흐름 이해로 문제 해결에 유용. 팀원들의 업무 파악과 작업 영향 이해가 필요하며 롤백 등 대비책 강조
      * 각종 변화 파악과 업무 방식 조율을 통해 사고 감소, 비즈니스 성공
    * 기업 문화에 따라 정책 시행 및 이행이 달라짐. 프로세스를 잘 알고 있는 기업은 새로운 프로세스를 도입하기 쉬우나, 그렇지 않은 경우 보다 어려움
      * 사람들이 정책 이행의 이점을 알기 쉽게 보여주고 관리진과 직원 모두가 이를 받아들이도록 함. 또한, 회의나 Confluence 웹사이트를 통해 주기적으로 정책 상기
      * 사고 대응 책을 작성하고, PagerDuty를 활용하여 해당 사고에 매칭되는 지침 제공. 이를 통해 구조화된 접근
      * 정책의 중요성 강조, 관련 문서를 통해 필요한 조치 상기, 필요에 따라 조정. 진행 상황을 파악하고 계속적인 피드백을 통해 개선
...
* [The Essence & Origins of Functional Reactive Programming • Conal Elliott • YOW! 2015 - YouTube](https://www.youtube.com/watch?v=rfmkzp76M4M)
...
* [**Streaming a Million Likes/Second: Real-Time Interactions on Live Video - YouTube**](https://www.youtube.com/watch?v=yqc3PPmHvrA)
  * [VidiGo Streaming a Million Likes/Second: Real-Time Intera](https://vidigo.ai//chatbot/summary/Cm8HQTFVpGATdZl)
    * 2. 실시간 플랫폼 구축과 도전
      ...
      * Unreal realtime team이라는 개발팀은 작은 문제부터 해결하며 점진적으로 아키텍처를 발전시켜 나가는 방식으로 실시간 플랫폼 구축
      * 서버와 클라이언트 간의 지속적인 연결 유지 방법과 akka 액터 모델을 활용한 다수의 연결 관리 방법 설명
    * 3. 실시간 이벤트 디스패치 시스템의 설계 및 최적화
      * 실시간 디스패처는 여러 프론트엔드 노드 사이에서 발행된 이벤트를 효율적으로 분배하는 역할
      * 프론트엔드 노드는 구독 중인 라이브 비디오에 대한 정보를 디스패처에게 전달하여, 적절한 노드로 이벤트가 전송되도록 조정
      * 디스패처의 병목 현상을 해결하기 위해 추가적인 디스패처 노드를 도입하고, 모든 디스패처가 접근 가능한 중앙 집중식 구독 테이블 사용
    * 4. 다중 데이터 센터 확장성 및 시스템 성능
      * 다중 데이터 센터 환경에서도 효율적으로 작동하도록 시스템 확장, 각 데이터 센터는 로컬 구독 정보를 유지하면서 다른 데이터 센터와 이벤트 교환
      * 단일 프론트엔드 노드는 최대 100,000개의 지속적 연결 처리 가능, 전체 시스템은 초당 백만 개의 좋아요를 처리
      * file descriptor 한계, 포트 고갈 등 다양한 스케일링 문제를 해결하며, 이러한 경험은 다른 개발자들과 공유
    * 5. 시스템 설계의 중요성과 확장성
      * 시스템의 디자인은 대규모 확장 가능
      * 디스패처 노드는 최대 프론트엔드 기계 수만큼 이벤트 게시
      * 프론트엔드 기계들이 처리하는 부분에 대해서는 걱정할 필요 없음
    * 6. 실시간 시스템의 성능과 지연 시간
      * 디스패처는 초당 5,000개 이벤트를 처리
      * 전체 시스템은 초당 50,000개의 좋아요 처리 가능
      * 종단 간 지연 시간은 75밀리초로 매우 빠름
  * [초당 백만 개의 좋아요 스트리밍: 라이브 비디오의 실시간 상호작용 | 완벽한 영상요약, 릴리스에이아이 | Lilys AI](https://lilys.ai/digest/385040?sId=yqc3PPmHvrA)
    * 1.📱KubeCon 참석자들에게 실시간 상호작용하는 플랫폼 소개 및 크리켓 경기 시청의 대세 중요성 강조
      * KubeCon에서 소개된 플랫폼 실시간 인터랙션 강조, 온라인 실시간 게시물에 참여하는 방법과 큰 실패 가능성을 가진 프로젝트 시작
      * 세계에서 가장 큰 라이브 스트림은 인도와 뉴질랜드 사이의 크리켓 월드컵 준결승, 영국 왕실 결혼식이 여러 나라로 확산되어 둘째로 큰 시청자 수를 기록
      * 개개인이 작은 것부터 시작하여 간단한 아키텍처 계층을 추가해 분산 시스템 문제를 해결할 수 있다는 신념
        * '실시간 플랫폼'을 개발한 경험에 대한 이야기, 리얼타임 제공의 중요성을 강조하며 공유
      * 이 플랫폼은 시청자들이 서로 상호작용하고 소통할 수 있는 기능 제공, 실시간 라이브 비디오 및 행사 전파와 사용자 간 상호작용에 초점
    * 2.🔗서버에서 클라이언트로 데이터를 스트리밍하는 방법 및 다수 연결 관리
      * 서버에서 클라이언트로 '좋아요'를 보내는 과정은 HTTP 요청으로 이뤄지며, 다시 수신자에게 되돌리기 위해 실시간 전송 시스템과 수신자 사이에 지속적 연결 필요
      * 이를 위해 클라이언트는 이벤트 소스 객체를 생성하고, 각 데이터 청크를 처리할 이벤트 처리기 정의
      * 또한 다수 연결을 관리하는 것이 다음 도전 과제로 제시, 수천 개의 연결을 관리하는 방법을 찾을 필요
      * HTTP롱 폴과 서버-이벤트를 사용하여 서버에서 클라이언트로 데이터를 스트리밍하는 방법 확인
    * 3.💼LinkedIn에서 Akka Actors를 이용해 연결 관리
      * Akka는 고도로 병행 및 메시지 기반 애플리케이션을 구축하기 위한 도구로 사용
      * Akka Actors는 상태와 행동을 갖는 객체로, 각 actor는 메시지 교환을 통해 통신
      * 각 actor는 메시지 처리 시 가벼운 스레드를 할당, 정의된 행동에 따라 상태 변경
      * LinkedIn에서는 이러한 Akka Actors 개념을 사용해 다수의 연결 관리. Play Framework과 Event Source API도 활용
    * 4.📡라이브 비디오 시청 중인 클라이언트에게 정확한 *좋아요* 전달 방법
      * 라이브 비디오를 시청 중인 클라이언트마다 정확히 좋아요를 전송하는 도전. 각 클라이언트가 다른 라이브 비디오를 보는 상황에서 어떻게 처리할지 고민
      * 구독 개념이 필요한데, 클라이언트는 현재 시청 중인 특정 라이브 비디오를 서버에 알릴 수 있도록 해야
      * 클라이언트들이 각각의 라이브 비디오 구독, 서버는 어떤 클라이언트가 어떤 라이브 비디오를 시청 중인지 파악하여 좋아요를 정확히 전송
      * 이를 통해 클라이언트가 현재 시청 중인 비디오에 대한 좋아요만을 받도록 보장. 클라이언트가 좋아요를 받는 방법에 대한 개념 소개
    * 5.💻많은 연결로 인한 확장 과제 해결과 실시간 이벤트 배포 과정 설명
      * 연결이 많아져 단일 기계가 처리할 수 없는 문제에 직면하게 되어 해결책을 찾는 과정 시작
      * 백엔드 엔지니어의 접근방법으로, 새로운 기계 추가 및 새로운 프런트엔드 서버들과 추진 이벤트 배포하는 실시간 디스패처 도입
      * 디스패처는 구독된 특정 라이브 비디오를 특정 프런트엔드 노드로 전달
      * 프런트엔드 노드들이 디스패처에 자신이 구독 중인 라이브 비디오를 알리고, 디스패처는 구독 테이블을 통해 정보를 관리하여 이벤트를 적절히 전달
    * 6.💻다수의 디스패처 노드 간 효율적인 통신을 위한 구조 설명
      * 시스템은 여러 프론트엔드 노드 간 분배를 하며 수많은 클라이언트에 연결되고, 연결 수를 비례적으로 확장
      * 디스패처가 시스템의 병목이 되며, 최대 확인 속도를 감당하지 못할 수 있어 추가 디스패처 노드를 도입하는 과정을 거침
      * 디스패처 노드는 프론트엔드 노드와 독립적이며, 구독 테이블은 디스패처 노드 간 공유 필요
      * 이를 위해 구독 테이블을 독립적인 키-값 저장소에 이식하여, 각 디스패처 노드가 언제든지 접근 가능하도록
      * 결과적으로, 구독 테이블은 디스패처 노드에 독립적이며, 클라이언트로의 영구적인 접속은 프론트엔드 노드에만 존재
    * 7.📹라이브 비디오 구독 및 분배 과정 설명
      * 라이브 비디오 시청자가 구독을 신청하면, 프론트엔드 노드가 해당 구독을 in-memory 구독 테이블에 저장
      * 프론트엔드 노드가 디스패처 노드에도 구독하기 위해 구독 요청을 보내고, 디스패처는 해당 정보를 키-값 저장소에 기록
      * 이렇게 전체 과정이 완료되면, 라이브 비디오에 대한 구독 과정이 마무리
    * 8.💬LinkedIn 실시간 플랫폼: 백엔드 처리 과정과 데이터 센터 추가에 따른 고민
      * 시청자가 실시간 영상을 좋아하기 시작하면, 백엔드 처리 과정 시작
      * 라이브 영상을 좋아하는 시청자들에 대한 정보는 likes 백엔드로 전송, 이를 구독 중인 프론트엔드 노드에 분배
      * 매우 단순하고 순차적으로 설계된 확장 가능한 분산 시스템을 통해 매 초 백만 개의 좋아요를 분산
      * 이 시스템은 LinkedIn의 실시간 플랫폼으로, 댓글, 타이핑 인디케이터, 확인 응답 등을 처리하며 실시간으로 LinkedIn의 온라인 상태를 관리하는 중요한 역할
      * 새로운 데이터 센터 추가로 인해 발생한 고민 속에서도 기존 아키텍처를 유지하면서 다중 데이터 센터에서 작동할 수 있는 방법을 모색
    * 9.📡다중 데이터 센터에서 좋아요 기능을 전달하는 프로세스
      * 빨간 라이브 비디오에 좋아요가 올라왔을 때, DC1에는 구독자가 없으나, DC3에는 있는 상황에서, 좋아요를 DC3의 시청자에게 전달하기 위한 방법
      * 좋아요를 받은 DC1에서의 프로세스를 설명하며, 다른 데이터 센터의 시청자들에게 좋아요를 어떻게 전달할지 고민
      * 크로스-데이터 센터 구독과 방송을 통해 다른 데이터 센터의 시청자들에게 좋아요를 전달하는 두 가지 방법을 제안하고, 이에 대한 장단점 언급
      * 다른 데이터 센터에서의 게시가 더 나은 방법임을 설명
    * 10.💻멀티 데이터 센터에서의 좋아요 분배 및 시스템 성능 향상 전략 공유
      * 피어 노드 전체에 피어 디스패처들에게 크로스 색상 또는 데이터 센터 게시를 수행하는 지점
      * 첫 번째 데이터 센터의 디스패처는 다른 모든 데이터 센터의 동료 디스패처에게 모든 좋아요 전달, 이 경우 DC3에서는 구독자를 발견, DC2에서는 발견하지 않음
      * 데이터 센터별로 구독 정보를 유지하면서 게시하는 동안 크로스-컬러 팬 아웃을 수행하여 글로벌 데이터 센터를 지원하는 플랫폼의 동작 방식을 보여줌
      * 프론트엔드 기계 당 10만 개의 연결을 저장할 수 있게되어 시스템의 성능을 높일 수 있었음을 실험을 통해 확인
        * LinkedIn에서 사용되는 시스템이 좋아요 분배뿐만 아니라 기타 모든 작업을 수행할 수 있는 것으로 확인
    * 11.👑로열 웨딩 라이브 스트리밍의 규모와 퍼포먼스 최적화
      * 로열 웨딩 라이브 스트리밍은 18백만 시청자가 있었으며, 180대의 머신으로 처리
      * 단일 머신 한 대가 10만개의 연결 처리, 180대의 머신으로 18백만 시청자에게 지속적인 연결 제공 가능
      * 디자인의 중요성 강조하며 디스패처 노드가 초당 이벤트를 5000개 발행 가능하며, 그것이 곱셈적으로 백만 명의 좋아요가 초당 분배되는 성능
      * 시스템의 최종 대기 시간은 p90에서 75밀리초
    * 12.💬LinkedIn 플랫폼 기술 설명 및 온라인 상태 확인 기술 소개
      * LinkedIn에서 'Samza' 기술을 이용한 시스템 구축 설명과 latencies 측정 방법 소개
      * 많은 기계에 걸쳐 end-to-end 시스템의 latencies 측정 및 확장성 전반적으로 discuss
      * 마찬가지로, 온라인 및 오프라인 상태를 파악하기 위한 'presence' 기술도 Platform에 구축
      * 실시간 콘텐츠 전달의 강점 및 사용법 설명, 및 서버 및 스토리지 기술로 Node.js, Python, Couchbase, Redis 등 활용 가능성 강조
    * 13.💻여러 질문에 대한 Guild의 서비스 및 이벤트에 대한 답변, 그리고 좋아요 동기화 질문에 대한 설명
      * 서버와 이벤트가 작동하지 않는 경우에 대한 보완책이 없는데, 대부분의 방화벽이 막지 않으며 대부분의 클라이언트가 이해
      * 비디오 스트림과 좋아요를 동기화하는 방법에 대한 질문에는, 좋아요 누른 순간 추적되고, 거의 즉각적으로 모든 시청자에게 표시
      * 이벤트의 일관성을 보장하기 위해 모든 데이터를 측정하지만, 속도가 중요하므로 완벽한 보장은 추구하지 않음
      * 만약 센드 모델에 문제가 발생하면 문제의 원인을 파악하고 수정
    * 14.💬카프카로 이벤트 전송할 때 속도와 확장성을 고려
      * 카프카를 사용하면 이벤트 전송을 보장하지만, 앞단 서버가 모든 라이브 비디오 토픽 소비 필요
      * 앞단 서버가 모든 이벤트를 소비하다 보니 확장성에 제한 발생, 연결 상태나 데이터 드랍 여부를 추적하면서 지연 발생
      * 앞단 서버는 클라이언트와 연결된 상태가 아님에도 데이터를 전달, 클라이언트 문제로 인해 이벤트가 제거되거나 수용되지 않는 경우에도 영향 없음
      * 속도를 추구하는 대신에 데이터 전송의 신속성이 우선. 데이터가 클라이언트로 정상 전달되었는지 확인하는 프로세스나 스레드 대기 없음
...
* [6년 묵은 레거시를 제거하며 앞으로의 6년 준비하기 - YouTube](https://www.youtube.com/watch?v=bqbJWDl8bfY)
  * [VidiGo 6년 묵은 레거시를 제거하며 앞으로의 6년 준비하기](https://vidigo.ai//chatbot/summary/539WISFiq71Ju8s)
  * [6년 묵은 레거시를 제거하며 앞으로의 6년 준비하기 | 완벽한 영상요약, 릴리스에이아이 | Lilys AI](https://lilys.ai/digest/386351?sId=bqbJWDl8bfY)
    * 7.💻첫 번째 전략: 레거시 다이어트 진행하며 코드의 히스토리 파악하기
      * 레거시 다이어트를 통해 레거시의 영향력을 줄이고 문제 해결 크기 감소
      * 코드 리팩토링 시 코드의 히스토리 파악이 중요하며, '체스터슨 울타리 원칙'을 적용하여 용도를 확인하고 결정 필요
      * 레거시 코드를 평가할 때는 존재 이유, 서비스 여부, 미래 사용 가능성을 고려하여 리포팅 필요
      * 레거시 다이어트를 통해 불필요한 코드 제거로 문제 감소를 이끌어냈으며, 이러한 과정을 통해 당연시되는 이점을 확인하며 전달
    ...
    * 9.📱개발자가 팀 문화와 효율성을 위한 전략 소개
      * 첫 번째 전략에서는 체스터슨 울타리 원칙을 강조하며, 두 번째 전략에서는 빠른 작업 공유가 중요함을 설명
      * 개발자는 개발과정에서 천재적이고 완벽한 결과를 바로 보여주고 싶어하는 욕심을 갖지만, 이로 인해 수정 비용과 효율성 문제 발생 가능성을 경고
      * 미리 구로나 문서로 방향성 공유하고 팀원 피드백을 받으면 작업 효율을 증가시키고 인터페이스 사용이 자연스럽게 이루어짐
      * 레거시 대응 과정에서도 효율을 유지하고, 빠른 롤백 환경 마련이 중요하며, 모바일 앱 개발 시 버전 롤백의 어려움을 강조하고 사용자 반응에 대비 필요
    * 10.💻코드 개발 중 기능 롤백을 가능케 하는 전략
      * 불가능한 버전 롤백은 아쉽지만, 실험 중에는 기능 롤백 가능
      * 앰플리튜드 플래그를 사용하여 배포 없이 즉시 기능 롤백이 가능하며, 안정적인 개발을 위한 전략으로 이용
      * 무사하게 리팩토링을 완료하기 위해 세 가지 전략을 적용하여 코드를 작성
      * 기능 분기를 염두에 두고 항상 코드를 작성했으며, 기능에 문제 발생 시 이전 기능을 사용할 수 있도록 대비
    * 12.💻 레거시에 도전하고 안정적인 소프트웨어의 미래를 고민하는 엔지니어들을 격려
...
* [RPC (Remote Procedure Call) 의 개념 :: Jins' Dev Inside](https://jins-dev.tistory.com/entry/RPC-Remote-Procedure-Call-%EC%9D%98-%EA%B0%9C%EB%85%90)
* [tRPC, gRPC, GraphQL or REST: when to use what? - YouTube](https://www.youtube.com/watch?v=veAb1fSp1Lk)
...
* grpc
  ...
  * [grpc-node: gRPC for Node.js](https://github.com/grpc/grpc-node?tab=readme-ov-file#grpc-reflection-api-service)
    * [grpc-reflection](https://github.com/grpc/grpc-node/tree/master/packages/grpc-reflection)
...
* [Architecting a Production Development Environment for Reliability - YouTube](https://www.youtube.com/watch?v=_aUAuZHuPeM)
...
* [Back to the future - how we integrated a 40 year old software system by Lukas Pradel - YouTube](https://www.youtube.com/watch?v=oMqqJuJt0PI) legacy
...
* [Your tests also need some architecting By Jonas Geiregat - YouTube](https://www.youtube.com/watch?v=T0p4FAJdYOQ)
...
* [Spotify's New Experimentation Platform (Part 2) - Spotify Engineering : Spotify Engineering](https://engineering.atspotify.com/2020/11/spotifys-new-experimentation-platform-part-2/)
* [Spotify’s New Experimentation Coordination Strategy - Spotify Engineering : Spotify Engineering](https://engineering.atspotify.com/2021/03/spotifys-new-experimentation-coordination-strategy/)
* [Changing the Wheels on a Moving Bus — Spotify's Event Delivery Migration - Spotify Engineering : Spotify Engineering](https://engineering.atspotify.com/2021/10/changing-the-wheels-on-a-moving-bus-spotify-event-delivery-migration/)
* [Comparing quantiles at scale in online A/B-testing - Spotify Engineering : Spotify Engineering](https://engineering.atspotify.com/2022/03/comparing-quantiles-at-scale-in-online-a-b-testing/)
...
* [Estimating long-term effects when only short-run experiments are available - Spotify Research : Spotify Research](https://research.atspotify.com/2023/04/estimating-long-term-effects-when-only-short-run-experiments-are-available/)
...
* [Deque Systems Inc.](https://github.com/dequelabs)
...
* [Voxxed Days Ioannina 2024 - Unlocking Collaboration in Code: A Journey through BDD - YouTube](https://www.youtube.com/watch?v=7wURMys8H9s)
  * [Voxxed Days Ioannina 2024 - Unlocking Collaboration in Code: A Journey through BDD - YouTube](https://lilys.ai/digest/344525?sId=7wURMys8H9s)
