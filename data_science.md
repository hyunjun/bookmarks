...
* [The Emerging Role of AI Data Engineers - The New Strategic Role for AI-Driven Success](https://www.dataengineeringweekly.com/p/the-emerging-role-of-ai-data-engineers)
  * [AI 데이터 엔지니어의 새로운 역할 | GeekNews](https://news.hada.io/topic?id=18811)
...
* [데이터 엔지니어링 전망: Data Engineering Weekly의 2025년 예상 | GeekNews](https://news.hada.io/topic?id=18402)
...
* ['데이터에 의한 의사결정'을 반대하는 사람은 없을 것이다. 하지만 실제로는 많은 회사에서 데이터 추출, 분석 및 활용은 굉장히 비효율적으로 진행된다. 왜 그럴까](https://www.linkedin.com/posts/hyungsuk-kim-6020628a_%EB%8D%B0%EC%9D%B4%ED%84%B0%EC%97%90-%EC%9D%98%ED%95%9C-%EC%9D%98%EC%82%AC%EA%B2%B0%EC%A0%95%EC%9D%84-%EB%B0%98%EB%8C%80%ED%95%98%EB%8A%94-%EC%82%AC%EB%9E%8C%EC%9D%80-%EC%97%86%EC%9D%84-%EA%B2%83%EC%9D%B4%EB%8B%A4-%ED%95%98%EC%A7%80%EB%A7%8C-%EC%8B%A4%EC%A0%9C%EB%A1%9C%EB%8A%94-activity-7235565656232845312-WVDw/)
  * 1. 사용하지도 않을 데이터를 너무 많이 쌓는다
  * 2. 비용이 커지고, 느려진다
  * 3. 스키마가 제대로 정의되어 있지 않다
  * 4. 요청하는 사람과 데이터를 추출하는 사람이 분리되어 있다
  * 5. 데이터를 제대로 해석하는 사람이 적다
  * 6. 대시보드가 많아진다
  * 7. 모두가 보는 단 하나의 대시보드가 없다
  * 처방
    * 전사 차원에서 관리하는 대시보드 숫자를 줄인다
    * 그 외의 데이터 추출은 뽑고 싶은 사람이 직접 하도록 한다
    * 활용하지 않을 데이터는 쌓지 않는다
    * 스키마를 정리하고, 빠르게 추출될 수 있도록 한다
...
* [NDC 25 불편 접수 후에야 대응? 넥슨은 조기 탐지로 운영한다](https://www.thisisgame.com/articles/214795) 데이터 드리븐 운영
...
* [DataJourney: Design first Open Source Data Management Toolkit](https://github.com/sayantikabanik/DataJourney)
  * [Statistics for sayantikabanik/DataJourney](https://sayantikabanik.github.io/DataJourney/)
  * [DataJourney - Design-First 오픈소스 데이터 관리 툴킷 | GeekNews](https://news.hada.io/topic?id=18431)
...
* [Python Data Science Handbook](https://github.com/jakevdp/PythonDataScienceHandbook)
  ...
  * [파이썬 데이터 사이언스 핸드북 | GeekNews](https://news.hada.io/topic?id=24812)
...
* [클릭률 그리고 추천 시스템, 기술과 가치의 조화 - 모두의연구소](https://modulabs.co.kr/blog/click-through-rate-recommendation/) differential privacy, federated learning
  * [연합 학습을 이용한 LINE 스티커 추천 -한국어판- - YouTube](https://www.youtube.com/watch?v=v-VwByrji0c)
    ...
    * [연합 학습을 이용한 LINE 스티커 추천 -한국어판- | 완벽한 영상요약, 릴리스에이아이 | Lilys AI](https://lilys.ai/digest/482966)
      * 1.️ 라인 스티커 기술을 활용한 머신 러닝 소개
        * 서버에서 머신 러닝 모델 학습 후 클라이언트에 전달하여 학습 결과 처리
        * 패딩된 러닝에서 클라이언트 측에서 학습 처리로 모델 배포 및 결과 통합 처리
        * 서버 측 머신 러닝과 온 디바이스 머신 러닝의 장단점 및 활용 방식 소개
      * 2. 클라이언트 러닝과 스티커 추천의 동작 원리 설명
        * 클라이언트 러닝은 클라이언트 상의 데이터를 처리하고 학습에 활용하여 서버로 보내는 네트워크를 줄여 프라이버시 보호
        * 스티커 자동 추천을 통해 '아리가또'나 '안녕하세요'와 같은 태그로 스티커를 선택할 수 있으며, 의미적 라벨을 활용하여 추천
        * 입력한 글자는 의미적 라벨링을 통해 스티커 추천에 활용, 'ㄱㄷ'로 시작하면 '아리가또', 'ㄴ' 추가하면 '안녕하세요' 스티커 추천
      * 3. 라인의 크리에이터즈 마켓: 태그 기능과 언어 다양성
        * 라인의 크리에이터즈 마켓을 통해 스티커와 태그 연동 확인 가능
        * 다국어 태그를 수동 또는 이미지 분석을 통해 정의하며, *일본어* 태그가 가장 다양함
        * 에스티커 프리미엄 서비스는 10백만 이상 스티커 패키지 중 사용자 선택 가능, 다운로드 불필요
        * 스티커 프리미엄은 *자동 다운로드*를 포함한 두 가지 종류의 스티커를 제공하며, *피보트 다운로드* 시스템 활성화
      * 4. 유저별 스티커 추천 및 랭킹 처리
        * 서버사이드에서 *캔디데이트 제네레이션*을 통해 유저별 스티커 추천을 처리
        * 클라이언트 측에서는 유저당 후보 생성 및 랭킹을 담당하며, *유저 임베딩*과 *아이템 임베딩*을 활용하여 추천 클러스터링을 실시
        * 이를 통해 유저의 취향을 고려한 *스코어*를 얻고, 적합한 스티커를 인퍼런스 처리하여 표시
        * *머신 러닝 모델*을 다운로드하여 처리하며, 디바이스 사용자가 어플리케이션을 사용하지 않을 때도 작동
      * 5. 머신 러닝이 캔디 데이트 스티커 선택 및 성능 향상에 대한 비교
        * 퍼스트 스테이지에서는 캔디 데이트 스티커를 선택함에 있어 머신 러닝 활용, 인포렌스 트레이닝은 서버 측에서 모든 처리 담당
        * 세컨드 스테이지 리랭킹에서는 스탬프, 인플레이션 여부, 클릭 정보 활용하여 퍼스널라이제이션 실시, 추론 처리 클라이언트 측에서
        * 마지막으로 페더레이티드 러닝 기법을 사용하여 클라이언트 쪽에서 대부분의 학습 처리를 하고, 모델 통합은 서버에서 진행
        * 플랫폼으로서 개발된 스티커 프리미어러는 애플리케이션 및 서비스에서 활용하도록 조치
          * 플랫폼 개발 시 중요한 요건은 애플리케이션 학습 및 개인정보 보호 기능의 공통화
      * 6. 시스템 구성 설명 및 구분, 구체적인 처리 과정
        * 시스템 구성 설명을 통해 수업 비즈니스 구현 및 머신러닝 처리를 서버와 클라이언트로 나누어 설명
        * 클라이언트 측에서 리랭킹 처리를 애플리케이션과 플랫폼을 통해 분담 설명
          * 사용자 입력에 따른 예측 요청과 처리과정, 클릭 정보의 처리 방법 설명
        * 데이터 전송, 정리 및 학습 시기 등의 처리과정에 대해 어플리케이션 측과 플랫폼 측 역할을 나누어 설명
      * 7. 로그 학습 및 플랫폼 지원 기능 소개
        * 로그가 클라이언트 애플리케이션 측에서 플랫폼 측으로 전송되고, 학습 및 모델 갱신
        * 플랫폼이 지원하는 기능으로 ABTS 도입, *user embedding*과 모델 정확성 유지를 위한 기능이 필요하다고 소개
        * 또한, 클라이언트 학습, 모델 학습 스케줄링, 그리고 iOS와 안드로이드 환경 모두를 지원하기 위해 오넥스런타임을 사용한다고 설명
        * 내일 예정된 추가 발표를 통해 더 상세한 내용이 소개될 예정이며, 한국 중재님께서 발표를 진행할 예정
      * 8. 플랫폼 개발에서 프라이버시 보호에 대한 접근방식
        * 플랫폼은 *서버 측 데이터*를 최소화하고 *차등 정보보호* 기술을 적용하는 두 가지 방법 사용
        * 로컬에서 발생한 로그를 학습한 후 *노이즈 추가*로 *개인정보 보호*를 강화
        * *페더레이티드 러닝*에서, *가우시안 메커니즘*과 *가중치 부여* 기술을 사용하여 사용자 식별 정보를 삭제하여 모델을 서버로 업로드
        * 결합된 모델은 정확한 학습을 보장하며, 작은 노이즈 적용으로 *개인정보 보호 파라미터*를 강화
      * 9. AB 테스트 결과와 개발 소개
        * AB 테스트 결과로 스티커 다운로드량 5.6% 향상, 사용자 업리프트 해석
        * 다국어 지원 시스템 개발에 전 세계의 엔지니어 30명 이상 협조
        * 향후 목표는 스티커 사용자 확대, 개인정보 보호 강화를 통한 서비스 향상
        * ldp 파라미터와 셔플링 기술 도입으로 개인정보를 더욱 효과적으로 보호할 계획
      * 11. 기술 개발 프로젝트에서 협력과 어려움
        * 기술적인 힘뿐만 아니라 수업 주기도 고려하여 스티커 기능을 개발
        * 플랫폼 개발 시 엔지니어들과 매니저의 협력이 중요하며 기술자 출신 매니저가 개발 리소스 할당
        * 페더레이트 러닝 기능 등 다양한 기술들을 조합하며 어려움은 프라이버시와 커뮤니케이션 부분, 협조를 통해 성공적으로 해결
      * 12.디바이스 간 데이터 연동과 유저 개인정보 보호 방안에 대한 논의
        * 디바이스 변경 시 *로그 데이터 계승* 가능, **클라이언트 측**에서 유저의 **인베딩 아이템**과 **모델**을 유지
