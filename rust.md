...
* [How We Replaced Crystal Reports with Rust & Typst (Sub-2ms PDFs) - YouTube](https://www.youtube.com/watch?v=9Ympl4XZjnE)
  * InfoQ. 컨설턴트 Erik Steiger가 Crystal Reports·Puppeteer·5GB LaTeX Docker 이미지 같은 레거시 PDF 생성을 Rust+Typst+AWS Lambda+Content-Addressable Storage 서버리스 파이프라인으로 대체한 사례. 렌더링 2ms 미만, 콜드스타트 메모리 50MB 미만, PDF 100만 건당 비용 50센트 미만, 문서의 Git 버전 관리까지
...
* [Dioxus | Fullstack crossplatform app framework for Rust](https://dioxuslabs.com/)
  * [dioxus: Fullstack app framework for web, desktop, and mobile](https://github.com/DioxusLabs/dioxus)
  ...
  * [Building ambitious software — Jonathan Kelley, Dioxus Labs & Cognition - YouTube](https://www.youtube.com/watch?v=H7vFrcNWXzs)
    * [3명이 5년간 손으로 쓴 30만 줄 Rust — 에이전트로 수만 줄을 뽑았는데 머지한 건 거의 없었습니다 - YouTube](https://www.youtube.com/watch?v=b3I_zBYF3e0)
    * Dioxus 창시자 Jonathan Kelley의 AI Engineer World's Fair 2026 발표. 코딩 에이전트 구독을 최대로 돌려 수년간 원했던 기능들을 수만 줄 Rust로 뽑았지만 머지 기준을 통과한 건 거의 없었다는 실전 경험—3명이 5년간 손으로 쓴 30만 줄 코드베이스에서 에이전트가 되는 것과 안 되는 것(AgentOS 한국어 정리 영상 포함)
